...
// Helper to group characters into words for proper text wrapping
// Words won't break mid-character, only at natural break points (spaces)
interface CharGroup {
  chars: Array<{ char: string; globalIndex: number }>;
  isSpace: boolean;
}

function groupCharsIntoWords(characters: string[]): CharGroup[] {
  const groups: CharGroup[] = [];
  let currentGroup: CharGroup = { chars: [], isSpace: false };

  for (let i = 0; i < characters.length; i++) {
    const char = characters[i];
    const isWhitespace = char === ' ' || /^\s$/.test(char);

    if (isWhitespace !== currentGroup.isSpace && currentGroup.chars.length > 0) {
      groups.push(currentGroup);
      currentGroup = { chars: [], isSpace: isWhitespace };
    }

    currentGroup.isSpace = isWhitespace;
    currentGroup.chars.push({ char, globalIndex: i });
  }
...
export default function TypingTest({ initialLanguage = "en", initialMode = 60 }: TypingTestProps) {
  ...
  const caretPosRef = useRef<{ left: number; top: number; height: number } | null>(null);
  const pendingAutoScrollRafRef = useRef<number | null>(null);
  const isAutoScrollingRef = useRef(false);
  const keystrokeTrackerRef = useRef<KeystrokeTracker | null>(null);
  // Guard to prevent multiple text extensions in rapid succession
  const isExtendingTextRef = useRef(false);
  ...
  // Throttle auto-scroll to prevent rapid triggers (chunk scrolling optimization)
  const lastScrollTimeRef = useRef<number>(0);
  
  // Ultra-fast typing detection (250+ WPM) for debounced scroll
...
Test yourself: `,
  ...
  const processInput = (value: string) => {
    ...
    let expectedG = expectedGRef.current.length ? expectedGRef.current : getGraphemes(text);

    // Ensure we always have enough buffered text while the timer is running.
    // This prevents fast typists from hitting end-of-text before time expires.
    const buildExtendedTextIfNeeded = (minRemainingGraphemes: number): string | null => {
      if (freestyleMode) return null;
      if (timeLeft <= 0) return null;
      const typedLen = typedG.length;
      let currentText = text;
      let currentExpected = expectedG;

      // If we already have enough remaining buffer, no-op.
      if (currentExpected.length - typedLen >= minRemainingGraphemes) return null;

      const MAX_APPENDS = 6; // hard cap to avoid runaway concatenation
      let appendedAny = false;

      for (let i = 0; i < MAX_APPENDS; i++) {
        const nextParagraph = getNextFromQueue() || (originalText || null);
        if (!nextParagraph) break;
        currentText = currentText ? `${currentText} ${nextParagraph}` : nextParagraph;
        currentExpected = getGraphemes(currentText);
        appendedAny = true;
        if (currentExpected.length - typedLen >= minRemainingGraphemes) break;
      }

      if (!appendedAny || currentText === text) return null;

      // Apply immediately so this keystroke can be processed against the extended text.
      setText(currentText);
      if (keystrokeTrackerRef.current) {
        keystrokeTrackerRef.current.expectedText = currentText;
      }
      return currentText;
    };

    if (typedG.length > expectedG.length) {
      // Try to extend synchronously (especially important in the last seconds of the timer)
      const extendedText = buildExtendedTextIfNeeded(300);
      if (extendedText) {
        expectedG = getGraphemes(extendedText);
      } else {
    ...
    // FIX: Removed startTransition - it caused race conditions with the useEffect[text]
    // that also updates charStates, leading to frozen UI and stale state
    setCharStates((prevStates) => {
    ...
    // CRITICAL FIX: Prevent multiple text extensions in rapid succession
    // Add time-based and flag-based guards to prevent cascading setText calls
    const shouldExtend = !freestyleMode && 
                         timeLeft > 0 && 
                         typedG.length >= expectedG.length - 100 && 
                         !isExtendingTextRef.current &&
                         now - lastExtendedAtRef.current > 500; // Minimum 500ms between extensions
    ...
    if (shouldExtend) {
      ...
      if (nextParagraph) {
        setText(prevText => {
          ...
          // Reset the flag after state update is applied
          requestAnimationFrame(() => { isExtendingTextRef.current = false; });
          return newText;
        });
        ...
        // No text available to extend with, reset flag
  ...
  const handleBeforeInput = (e: React.FormEvent<HTMLTextAreaElement>) => {
    ...
    if (inputData && inputData.length > 0) {
      ...
      if (!expectedChar) {
        // If time remains, attempt to extend text instead of blocking fast typists.
        if (!freestyleMode && timeLeft > 0) {
          // Trigger an immediate extension; processInput will also handle any overflow safely.
          const nextParagraph = getNextFromQueue() || (originalText || null);
          if (nextParagraph) {
            setText(prevText => {
              const newText = prevText ? `${prevText} ${nextParagraph}` : nextParagraph;
              if (keystrokeTrackerRef.current) {
                keystrokeTrackerRef.current.expectedText = newText;
              }
              return newText;
        ...
        // No buffer available: block to prevent overflow beyond available text
        e.preventDefault();
  ...
  // FIX: Removed useDeferredValue and startTransition - they caused race conditions
  // that made charStates lag behind wordGroups, freezing the UI on text extension

  // Memoize word groups with character states to prevent recalculation on every render
  // Uses charStates directly now for immediate color feedback
  ...
  const handleCaretPositionChange = useCallback((pos: { left: number; top: number; height: number }) => {
  ...
  useCursorPosition({
    ...
    onPositionChange: handleCaretPositionChange,
  });

  // Keep caret visible within the scroll container during typing
  // Ultra-robust scroll handling: supports 250+ WPM with debounced scrolling
  useEffect(() => {
    const container = containerRef.current;
    const caretPos = caretPosRef.current;
    if (!container || !caretPos) return;
    if (isUserScrollingRef.current) return;

    // Chunk scrolling configuration - maximized for more visible lines
    ...
    // Track keystroke timestamps for WPM detection (keep last 10)
    ...
    // Ultra-fast: < 50ms (250+ WPM) - use debounce, no immediate scroll
    // Fast: 50-80ms (150-250 WPM) - long throttle
    // Normal: > 80ms (< 150 WPM) - standard throttle
    const isUltraFast = avgInterval < 50;
    const isFast = avgInterval < 80;
    
    // For ultra-fast typing, use debounce pattern - only scroll after pause
    if (isUltraFast) {
      ...
      // Schedule scroll for after typing pause (150ms)
      scrollDebounceRef.current = setTimeout(() => {
        scrollDebounceRef.current = null;
        
        const currentCaretPos = caretPosRef.current;
        if (!currentCaretPos || !container) return;
      ...
      return; // Don't do immediate scroll for ultra-fast typing
    }
    
    // For fast/normal typing, use throttle-based approach
    const MIN_SCROLL_INTERVAL = isFast ? 500 : 300;
    if (now - lastScrollTimeRef.current < MIN_SCROLL_INTERVAL) return;
    ...
    pendingAutoScrollRafRef.current = requestAnimationFrame(() => {
      ...
      const caretTop = caretPos.top;
      const caretBottom = caretPos.top + caretPos.height;

      let targetTop: number | null = null;

      // Handle upward scroll (keep small margin for quick response)
      ...
      // Handle downward scroll with chunk-based approach
        ...
        if (caretBottom > visibleBottom - triggerThreshold) {
          ...
          const targetPosition = containerHeight * SCROLL_TARGET_POSITION;
          targetTop = Math.max(0, caretPos.top - targetPosition);
        }
      }
      ...
      if (targetTop !== null && Math.abs(targetTop - container.scrollTop) > 10) {
        lastScrollTimeRef.current = performance.now();
        isAutoScrollingRef.current = true;
        // Use instant scroll during typing to prevent animation stacking and lag
        container.scrollTo({ top: targetTop, behavior: 'auto' });
        // Reset flag immediately since scroll is instant
        isAutoScrollingRef.current = false;
      }
    });
  }, [typedLength, text.length]); // Also trigger scroll recalc when text extends
            ...
            <div className="flex items-center gap-1.5">
              <Tooltip>
                ...
                <TooltipContent
                  ...
                  sideOffset={8}
                  collisionPadding={8}
                  className="pointer-events-none max-w-xs bg-popover text-popover-foreground border shadow-lg p-3"
                >
              ...
              <Tooltip>
                ...
                <TooltipContent side="bottom" align="start" sideOffset={8} collisionPadding={8} className="pointer-events-none max-w-[280px]">
        ...
        <div className="relative min-h-[200px] md:min-h-[300px] max-h-[300px] md:max-h-[400px] overflow-hidden group">
          ...
          {!freestyleMode && text && (
            <div
              ...
              className={cn(
                "relative w-full h-full max-h-[300px] md:max-h-[400px] overflow-y-auto p-4 md:p-8 text-lg sm:text-xl md:text-3xl font-mono leading-relaxed outline-none transition-all duration-300 select-none",
                // Only use smooth scroll when not actively typing to prevent animation stacking
                !isActive && "scroll-smooth",
                // Use word-wrap for proper wrapping at word boundaries, not mid-word
                "break-normal",
            ...
            >
              ...
              {wordGroupsWithStates.map((group, groupIndex) => {
                ...
                // Word group - wrap in nowrap span to prevent mid-word breaks
                return (
                  <WordSpan
                    key={`word-${groupIndex}`}
                    chars={groupChars}
              ...
              {/* GPU-accelerated caret - positioned via transform for smooth movement */}
              {!isFinished && (
                <div
                  ref={caretRef}
                  className={cn(
