...
async function withDbRetry<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`[DB Retry] Attempt ${attempt + 1} failed, retrying in ${delayMs}ms...`);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // In development, generate a random session secret if not provided
  // This ensures no hardcoded secrets exist in the codebase
  const sessionSecret = process.env.SESSION_SECRET || (() => {
    if (process.env.NODE_ENV === "development") {
      console.warn("⚠️  No SESSION_SECRET set - using random key (sessions won't persist across restarts)");
  ...
  app.get("/api/typing/paragraph", aiGenerationLimiter, async (req, res) => {
    try {
      ...
      // Check for exact match first (no fallbacks)
        ...
        paragraph = await withDbRetry(() => storage.getExactParagraph(language, mode, difficulty));
        if (paragraph) {
      ...
      if (!paragraph && generateIfMissing && mode && !customPrompt) {
        ...
        try {
          ...
          paragraph = await withDbRetry(() => storage.getRandomParagraph(language, mode, difficulty));
          if (paragraph) {
  ...
  app.get("/api/typing/paragraphs/batch", async (req, res) => {
    try {
      ...
      // Use efficient batch fetch with single query and shuffle (with retry for transient errors)
      const paragraphs = await withDbRetry(() => storage.getRandomParagraphs(language, count, mode, difficulty));
  ...
  // Script validation patterns for detecting corrupted paragraphs
  const SCRIPT_PATTERNS: Record<string, { expected: RegExp; forbidden: RegExp[] }> = {
    ar: {
      expected: /[\u0600-\u06FF]/, // Arabic script
      forbidden: [/[\u0900-\u097F]/, /[\u0980-\u09FF]/, /[\u0B80-\u0BFF]/, /[\u0C00-\u0C7F]/] // Devanagari, Bengali, Tamil, Telugu
    },
    hi: {
      expected: /[\u0900-\u097F]/, // Devanagari
      forbidden: [/[\u0600-\u06FF]/, /[\u0980-\u09FF]/] // Arabic, Bengali
    },
    mr: {
      expected: /[\u0900-\u097F]/, // Devanagari
      forbidden: [/[\u0600-\u06FF]/, /[\u0980-\u09FF]/] // Arabic, Bengali
    ...
    bn: {
      expected: /[\u0980-\u09FF]/, // Bengali
      forbidden: [/[\u0600-\u06FF]/, /[\u0900-\u097F]/] // Arabic, Devanagari
    },
    ta: {
      expected: /[\u0B80-\u0BFF]/, // Tamil
      forbidden: [/[\u0600-\u06FF]/, /[\u0900-\u097F]/] // Arabic, Devanagari
  ...
  app.get("/api/admin/typing/validate-paragraphs", isAuthenticated, async (req, res) => {
    try {
      ...
      const corruptedParagraphs: Array<{ id: number; language: string; content: string; issue: string }> = [];

      // Get all paragraphs for the specified language or all non-English languages
      const languagesToCheck = language ? [language] : Object.keys(SCRIPT_PATTERNS);

      for (const lang of languagesToCheck) {
        const patterns = SCRIPT_PATTERNS[lang];
        if (!patterns) continue;

        const paragraphs = await storage.getRandomParagraphs(lang, 100); // Check up to 100 per language
        
        for (const p of paragraphs) {
          // Check for forbidden scripts
          for (const forbiddenPattern of patterns.forbidden) {
            if (forbiddenPattern.test(p.content)) {
              corruptedParagraphs.push({
                id: p.id,
                language: p.language,
                content: p.content.substring(0, 200) + "...",
                issue: "Contains mixed scripts"
              });
              
              // If fix=true, mark the paragraph as typing-related to exclude it
              if (fix) {
                await storage.markParagraphAsTypingRelated(p.id);
                console.log(`🔧 Marked paragraph ${p.id} (${lang}) as typing-related to exclude it`);
              }
              break;
            }
          }
      ...
      res.json({
        corruptedCount: corruptedParagraphs.length,
        fixed: fix,
        corrupted: corruptedParagraphs
      });
    } catch (error: any) {
      console.error("Validate paragraphs error:", error);
      res.status(500).json({ message: "Failed to validate paragraphs" });
    }
  });
