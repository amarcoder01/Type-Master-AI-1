...
export interface IStorage {
  ...
  markParagraphAsTypingRelated(paragraphId: number): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async markParagraphAsTypingRelated(paragraphId: number): Promise<void> {
    await db
      .update(typingParagraphs)
      .set({ isTypingRelated: true })
      .where(eq(typingParagraphs.id, paragraphId));
  }
